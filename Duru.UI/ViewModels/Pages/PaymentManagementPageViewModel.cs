...
public partial class PaymentManagementViewModel : ObservableObject
{
    ...
    public List<string> StatusFilters { get; } = new() { "Tümü", "Ödendi", "Kısmi Ödendi", "Ödenmedi", "İade Edildi" };
    ...
    private void ApplyFilter()
    {
        ...
        if (FilterStartDate.HasValue) filtered = filtered.Where(p => p.PaymentDate.Date >= FilterStartDate.Value.Date);

        if (FilterEndDate.HasValue) filtered = filtered.Where(p => p.PaymentDate.Date <= FilterEndDate.Value.Date);
        ...
        if (SelectedStatusFilter != "Tümü") filtered = filtered.Where(p => p.Status == SelectedStatusFilter);
